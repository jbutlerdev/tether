...
//
//   mic → I2S → Opus encode → PSRAM ring → SD card (store-and-forward)
//   SD card → radio_task → LoRa TX (34-byte header, fragment + ACK)
...
// The audio buffer: PTT press → capture into PSRAM ring → PTT release
// → flush to SD → enqueue to radio_task.
...
// Recording buffer: accumulates Opus frames during PTT.
...
void OnIncomingPacket(const tether::m5::IncomingPacket &pkt) {
  if (pkt.header.msg_type == tether::m5::MsgType::kTtsData) {
    // TTS audio chunk — play it on the amp. For v1 we just buffer
    // and play when TTS_END arrives; a full streaming player is v2.
    // TODO(v2): stream TTS chunks to the amp as they arrive.
...
extern "C" void app_main(void) {
  ...
  // 5. PSRAM ring + Opus encoder + audio capture.
  ...
  // 12. Main loop: feed the watchdog + drive PTT state.
  for (;;) {
    vTaskDelay(pdMS_TO_TICKS(100));
    // The PTT state machine transitions are driven by the button
    // callback; the radio task's ACK / failure signals are driven
    // by radio_task::Step(). Here we just keep the main task alive
    // for ESP_LOG and display updates.
  }
}
