...
// Test 1: Channel 0 maps to 902.3 MHz (US915 uplink ch 0, per research.md
// §6.2). US915 has 64 uplink channels of 125 kHz each starting at 902.3 MHz, so
// ch 63 is at 902.3 + 63*0.125 = 910.175 MHz. (plan.md §3.2 listed
// 914.9 MHz for ch 63, which is the 200-kHz-step value and is not the
// US915 spec; we use the correct US915 value here.)
void test_lora_set_channel_frequency() {
  ...
  TEST_ASSERT_TRUE(ch1.has_value());
  TEST_ASSERT_EQUAL_UINT64(902'425'000ULL, ch1->frequency_hz);

  auto ch63 = Channel::FromIndex(63);
  TEST_ASSERT_TRUE(ch63.has_value());
  TEST_ASSERT_EQUAL_UINT64(910'175'000ULL, ch63->frequency_hz);
...
void test_lora_set_channel_calls_set_frequency() {
  ...
  radio.SetChannel(63);
  TEST_ASSERT_EQUAL_UINT64(910'175'000ULL, mock->last_frequency_hz);
}
